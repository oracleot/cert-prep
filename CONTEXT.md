...
### Answer Option
One selectable answer presented with a Rex prompt.
...
A Rex prompt with Answer Options where more than one option is correct. A correct Learner Response must select the exact full set rather than receiving partial credit. The learner is told that more than one option should be selected.
