...
### Learner Response
The learner’s selected Answer Option or Answer Options before evaluation. It is selection-only rather than free-text, must include at least one selection, and may be changed before submission without penalty. After submission, it is locked for that prompt.

### Sage Reply
The system’s response after evaluation. While it streams, the prompt remains visible with the learner’s chosen options marked. For option-based prompts, it explains the correct option or options first with the most depth, then briefly explains each distractor. It refers to options by label plus short paraphrase rather than repeating each full option text. On Multiple-Response misses, it explicitly names any missed correct option and any incorrectly chosen option.
