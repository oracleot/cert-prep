...
import { DEFAULT_MODELS, loadSettings, saveSettings, type AgentModelKey, type AppSettings } from "@/lib/settings";
...
function modelLabel(key: AgentModelKey) {
  return key === "rex" ? "Rex" : key === "sage" ? "Sage" : "Evaluator";
}
...
export function SettingsClient() {
  ...
  async function rebuildCurriculum() {
    ...
    const res = await fetch("/api/settings/learning-style", {
      ...
      body: JSON.stringify({ user_id: getAnonymousUserId(), learning_style: settings.learningStyle }),
    });
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mt-8 rounded-[2rem] border border-zinc-200 bg-zinc-950 p-7 text-zinc-50 dark:border-zinc-800">
          ...
          <h1 className="mt-4 max-w-3xl text-4xl font-black tracking-tight sm:text-6xl">
            Tune the gauntlet without sanding off the edge.
          </h1>
        ...
        <section className="mt-5 grid gap-5 lg:grid-cols-2">
          <div className="rounded-[2rem] border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-950">
            ...
            <input
              ...
              min="1"
              max="5"
              value={settings.sessionCycles}
              onChange={(event) => updateSetting({ sessionCycles: Number(event.target.value) })}
              className="mt-2 h-12 w-28 rounded-xl border border-zinc-300 bg-transparent px-3 font-black dark:border-zinc-700"
            />
          ...
          <div className="rounded-[2rem] border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-950">
            ...
            <select
              ...
              onChange={(event) => updateSetting({ learningStyle: event.target.value as LearningStyle })}
              className="mt-5 h-12 w-full rounded-xl border border-zinc-300 bg-transparent px-3 font-bold dark:border-zinc-700"
            >
          ...
          <div className="rounded-[2rem] border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-950">
            ...
            <input
              ...
              onChange={(event) => updateSetting({ openRouterKey: event.target.value })}
              placeholder="sk-or-v1-..."
              className="mt-4 h-12 w-full rounded-xl border border-zinc-300 bg-transparent px-3 dark:border-zinc-700"
            />
          ...
          <div className="rounded-[2rem] border border-rose-300 bg-rose-50 p-6 dark:border-rose-900 dark:bg-rose-950/30">
            ...
            <Button onClick={openResetDialog} variant="destructive" className="mt-5">
              Reset progress
            </Button>
          </div>
        </section>
