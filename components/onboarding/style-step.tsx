...
export function StyleStep({ value, onChange, onBack, onStart, isLoading }: Props) {
  return (
    <section className="rounded-[2rem] border border-zinc-200 bg-white/90 p-7 sm:p-10 dark:border-zinc-800 dark:bg-zinc-950/85">
      <p className="text-xs font-semibold uppercase tracking-[0.4em] text-emerald-600 dark:text-emerald-300">
        Learning style
      </p>
            ...
            onClick={() => onChange(style.value)}
              className={`min-h-24 rounded-3xl border p-5 text-left transition ${
                value === style.value
                  ...
                  : "border-zinc-200 bg-white text-zinc-700 hover:border-zinc-400 dark:border-zinc-800 dark:bg-black/60 dark:text-zinc-200 dark:hover:border-zinc-500"
              }`}
            >
